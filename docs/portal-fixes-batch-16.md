...
> **Shipped, 6/6 E2E green (through the real admin API).** (1) `materializeSessions`' taken-set now includes `rescheduled`/`cancelled` — identical semantics to the monthly cycle's PL-62 rule, so a vacated slot is never resurrected (and never re-billed on top of a tombstone's late fee). (2) `clearFutureSessions` **preserves replacement sessions** (any session that is the target of a tombstone's `rescheduled_to_id`) — chosen over cascade-with-fee-reversal exactly as the doc suggested: preserving is simpler and matches the monthly cycle's semantics; a replacement is an agreed commitment, not generated filler. (3) **Repair pass — one doc-premise correction:** `rescheduled_to_id` carries an FK with `ON DELETE SET NULL`, so literally dangling ids cannot exist (verified: all 5 links in the database resolve); the real damage was the SILENT NULLING of the chain when a replacement got deleted — which (2) now prevents. Nothing to repair, prevention is the fix. Verified end-to-end: materialize → reschedule one slot to +24h → regenerate → the tombstone stays the only row at the vacated time, the replacement survives confirmed, the chain link is intact, and no time has duplicate live sessions.
