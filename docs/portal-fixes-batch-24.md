...
**✅ SHIPPED (Jul 30).** The section is **Schools** in both homes (Classes tab + Contacts tab; section id stays `contacts` so every existing deep link works). Each school is a card: nickname + full name + timezone + logo at a glance, "edit school" opens an inline editor for **full name, nickname, timezone (PL-238 alias picker), logo upload (same background-removal route), accent color, collateral language** — the same `schools` columns the wizard and Branding-defaults panel read; one source of truth, now three entry points. **The contacts are an attribute:** each card lists its active affiliations (name, mailto email, since, digest select, Edit/Move/End — all the existing machinery, unchanged) plus a per-school "+ add a contact" inline form; a school with nobody active says plainly "no active contact — the school gets no digests and nobody can open its portal"; past affiliations stay as the grouped toggle list. **Names are doors:** `?school={id}` lands on the card (scroll + highlight via the standing focus machinery), and the Branding-defaults panel's school names now link there. **Edit-safety, as discovered:** there is **no school-level slug column** — class slugs/registration URLs are minted at class creation from the nickname and never rewritten, and hgl.co short links are per-class — so a nickname change only affects FUTURE classes and collateral text; the editor shows exactly that warning when (and only when) the nickname is edited, and display-name changes are safe by construction. **The doc's acceptance case is done as real data: "ASF – ASF" was renamed to "American School Foundation" (nickname ASF) through the new editor itself** — verified in the DB and on the card. One wiring fix along the way: school edits now refresh the parent's schools list, not just the counselor roster.
