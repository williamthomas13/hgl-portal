...
> **Shipped.** (a) The wizard (the only place minimums are set — there is no separate admin-edit surface) now blocks any minimum below 1: the input enforces `min=1`, blank falls back to the mode default, and the save handler rejects the rest — the old `Number(x) || default` happily saved `-1`, which is how Cape Town got its row. (b) Non-blocking amber warning under the field (and live as you type) when in-person min < 8 or online min < 3. (c) Cape Town repaired: `-1 → 8` (in-person default); zero classes with min < 1 remain. (d) `loadClassBundles` now sanitizes any nonsensical stored minimum to the mode default at load time, so the roster report, the min-met alert, and every verdict computation can never render "min -1 met" again regardless of what's in the row.
