# Portal fixes — batch 50 (CLOSED for hand-off Sep 21, 2026 — 6 items, PL-461…466)

**Standing rules:** all prior, plus the PL-460 rule (every call-to-action lands where the action can be done).

(Opened + closed Sep 21, 2026. Next PL: PL-467.)

**ORDER: the cutover waits on PL-463, PL-464, PL-465 — ship those three FIRST and say so in the header note the moment they are on prod.** Then PL-466, PL-461, PL-462. Code does NOT run any import against prod — Scarlett + Claude run the dry runs and the imports themselves.
