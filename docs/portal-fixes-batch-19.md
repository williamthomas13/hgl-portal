...
**✅ SHIPPED (Jul 27) — one config step needs you/Kelsie (below).** Four pieces:
- **Takes over the EXISTING calendar in place:** new `intl-calendar.ts` writes to whatever shared calendar `intl_classes_calendar_id` names (configured on the Google Calendar panel — paste the id everyone already subscribes to and Save; owner defaults to billy@). Class-level SPAN event (all-day, first→last session — the commitment/travel window) + per-session blocks, colored through the same `calendar-colors.ts` map as PL-160 using the Google palette ids the hand calendar used. Status transitions drive it: the daily cron syncs + audits, and cancel-class recolors RED immediately via after() — never deletes, matching the March-screenshot practice. Migration `20260818000002` (applied) adds the event-id + last-written-hash anchors.
- **One-time reconciliation (PL-154 machinery):** "adopt hand-made events" on the GCal panel — timed events matching a portal session's exact start+end adopt as that session; all-day events overlapping a class window with a resembling title adopt as its span (subsequent updates flow — the next sync takes the adopted event over into portal shape); everything unmatched is listed in an on-screen report for Kelsie. Nothing is ever silently deleted.
- **Hand edits detected, never overwritten:** every write records a hash of what the portal wrote; the sync only patches when the PORTAL side changed, and the daily drift audit compares live events against the last write — a hand edit produces an ops alert naming the event and the difference ("title is now …, time moved — portal not overwriting").
- **Instructor-fit suggester, advisory only:** `/api/admin/instructor-fit` checks every ACTIVE instructor (PL-176 flag honored) against (a) Google busy via the same freebusy machinery as the tutor-week grid, (b) portal commitments including PL-159 holds, and (c) in-person travel-window overlaps — ranked, conflicts named plainly, and an instructor with a hard session-time conflict is NEVER ranked available. "Who's free to teach it?" on every class card opens the PL-160 calendar with the suggestion panel; "show on calendar" overlays that candidate's busy time (gray) against the class's purple-outlined session blocks — Kelsie sees exactly what the ranking saw and can disagree. Assignment stays her explicit click on the class page. Candidates whose Google couldn't be checked say so ("portal data only") rather than feigning certainty.

**Verified:** 12-check fake-Google E2E (sync creates span+sessions with correct colors · idempotent · cancel recolors red IN PLACE, ids stable · drift audit reports a hand edit and the sync leaves it alone · reconciliation adopts by time and by title, reports the "Dentist" stranger untouched · adopted events flow into portal management) — plus live UI: the Nido class's suggester ranked Billy "conflicts with 1 session — busy per Google Calendar at Aug 20" (correctly not available) and the overlay drew his 10 real busy blocks against the outlined class sessions. **Needs you:** paste the real International Classes calendar id into the GCal panel and press "adopt hand-made events" once — everything no-ops safely until then.
...
**✅ SHIPPED (Jul 27).** They were already ONE record (both pages edit the `instructors` table — subjects/timezone/offer-windows from the Tutors panel, name/comms from the Instructors page) — but the "default Zoom link" concept lived in TWO columns: `default_location` (Tutors panel; every real link — all 7 tutors' Zoom URLs — was here) and `default_meeting_link` (Instructors page; empty everywhere). That's exactly why billy@'s link surfaced in the 1-on-1 flow but the Instructors list said "none". Unified with a migration per the standing lesson, not a sync: `20260818000003` (applied) copies the links into `default_meeting_link`, every reader repointed (1-on-1 wizard prefill, parent portal location fallback, GCal event location, welcome-handoff email, both panels), and the deprecated column drops via `20260818000004` once this deploy is live (dropping in the same step would 500 the still-running old build). Labels stay position-appropriate per surface ("Default Zoom link" both places now — it was never an address in practice). Verified live: the Instructors list now shows Billy's Zoom link and editing it there landed in the ONE column the Tutors panel reads; no duplicate records existed to orphan.
