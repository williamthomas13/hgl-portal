...
✅ **Shipped Aug 7 — fully built and verified; the six templates are DRAFTS awaiting YOUR flip, which IS the go-live switch.** The send engine renders registry-only (no code twin — deliberately: one source, zero drift), so a draft renders nothing and nothing can send until you set the FO templates live on the templates page. **Your sign-off surface: Settings → View as → Parent** — all six render there through the editor's own sample pipeline (subjects + full bodies; screenshot-checked). Review, tweak in the editor if you like, flip live → the next hourly sweep arms every eligible cohort.

**What's live underneath** (migration `20260830000001` APPLIED: `classes.fo_short_name` + `classes.fo_extended_until`):
- **Per-cohort rolling windows** anchored on each feeder's OWN last session: announce +1 day · deadline +14 · reminder = deadline −2 · extension +7 (constants in `follow-on-shared.ts`, one source for the sweep, the checkout seam, AND the roster display — numbers still yours to change). {endDate} composes per cohort, extension-aware.
- **The sweep leg** (`sweepFollowOnForBundle`, on the hourly sweep): audience = Paid/Completed feeder enrollments whose class points at an OPEN class with a complete promo (code + amount — PL-266 completeness); parent + student pairs; **family marketing opt-out honored**; **suppressed once registered** in the follow-on class (any live row incl. a pending checkout, re-checked every stage, so registering mid-sequence stops the rest); sendOnce dedupe per stage+enrollment.
- **ONE shared code, portal-validated per cohort**: the open class's existing `promo_code`/`promo_amount`. **Tokenized links preferred** — every email's {followOnRegistrationLink} is that family's own signed link (`fo:` prefix, addon-page pattern) which auto-applies the discount on the register page (green banner, struck-through price, discounted checkout button) with THEIR deadline enforced server-side; the typed code is the fallback (a "Discount code" field appears on classes with a promo; validated at checkout against the family's own cohort by parent email — wrong code, stranger email, and expired cohort all refuse with plain-English reasons BEFORE any charge). The checkout discount seam reduces the class line server-side (Stripe line reads "…(CODE — $50 off)"), so snapshots, receipts, and QBO all see the true paid price with zero new plumbing.
- **The feeder roster card** shows "Follow-on discount window: ends {date}" + the **extend-a-week** action (inline confirm); open classes get a "Short marketing name" inline edit ("Deep Dive").

**Recommendation — stage-3 trigger: ADMIN ACTION, not auto-extend (built accordingly).** The "Bad News, Great News" copy narrates a deliberate business decision ("we extended the discount") — auto-extending every cohort would make the urgency fake and train families that deadlines here are soft. The extend button is one click per cohort on the feeder card, sets `fo_extended_until` (+7 from the current deadline, or from today if long past), and the next sweep sends the pair. If you later want auto-extend, it's a small sweep addition — say so and it ships behind a per-class switch.

**Recommendation — how the Deep-Dive phrasing degrades**: the standalone italicized *Deep Dive* became *{followOnShortName}* (new roster-editable "short marketing name" on the open class; blank = full class name) — your voice untouched, and a future follow-on gets its own short name for free. Two sentences stay deliberately literal because they're course-specific claims, not template scaffolding: FO-1's "…designed for students who have already taken our SAT class … or who already have an SAT math score of 650 or above" and FO-2S's "our upcoming Advanced SAT Math course". For a different follow-on, those are the only two spots to re-edit (templates are versioned — a one-minute editor pass). Formatting-only adaptations to your copy: links render clickable via the [x](x) pattern, and preheaders are empty (none existed in your sends; add them in the editor if you want them).

**Verified 31/31 on the real machinery** (`scripts/verify-pl279-fo.mjs`, send-light, QA fixtures fully cleaned): cohort math; drafts render nothing; flipped live → announce pair renders with the cohort's endDate/code/short-name/tokenized link, zero unresolved variables; suppression; extension arms stage 3; token + typed-code + forged-token + wrong-code + stranger-email + expired-cohort all behave. Also deliberate: FO rows are event-driven like waitlist offers (not projected into the Upcoming tab — sends appear on the comms timeline with body snapshots once sent); FO sends ride the relationship footer (visible unsubscribe) from billy@, matching your manual sends.
...
📋 **PLAN reported Aug 7 (nothing built, per the item). Four phases, ~2.5–3 days total, in the order below — each ships independently.**

**Phase A — the family-history record + segment chips (~1 day, the core).** Replace v1's ad-hoc `FamilyFacts` with one `familyHistory(familyId)` resolver in `campaigns.ts` producing everything-we-know, computed from the SAME columns the trusted surfaces already read (nothing duplicated): **classes** — per enrollment: class type, school, dates, outcome (Completed / Paid-current / cancelled / refunded / waitlisted-never-enrolled); **tutoring** — subjects, engagement status (active vs lapsed = no session in N months, N a chip parameter), package hours bought/used/remaining (the PL-197 uncapped math), rate band; **financial** — lifetime paid (class price snapshots + add-on price_paid + paid tutoring invoice totals — the exact PL-204 term-report columns, so campaign money math can never disagree with the report), purchase count, outstanding/past-due invoices, credits, refunds, promo codes used (checkout discounts + FO codes). New chips stay plain-English and AND-composable on the existing jsonb `SegmentDef` (existing saved definitions keep resolving — additive only): "took {class type} at {school}" with an outcome + date-range qualifier · "never bought tutoring" / "tutoring lapsed {N}+ months" / "active tutoring" · "hours remaining under {N}" · "spent over ${X}" / "across {N}+ purchases" · "no outstanding balance" / "has a past-due invoice" · "used a promo code" · "was refunded". Both of the item's examples compose directly. **Preview**: same resolve-then-refuse-if-changed flow; the WHY column speaks the history plainly ("took SLS SAT Prep (completed Oct 2026) · never bought tutoring · $0 outstanding"). **The financial firewall is structural, gate-asserted**: financial facts exist ONLY in `SegmentDef`/WHY — no dollar-shaped variable will be added to the composer vocabulary, so financial data cannot render into copy even by template edit (the save-time unknown-variable block enforces it); `regress:campaigns` gains a PL-204-style assertion that the campaign render vocabulary contains zero dollar-shaped variables and that a preview's recipient payload carries no raw amounts beyond the WHY strings Scarlett composed.

**Phase B — full composer variables (~½ day).** Route campaign rendering through `resolveVariables` instead of the current five hard-coded substitutions: per recipient, synthesize an `EnrollmentEmailContext` the way `tutoringStubContext` already does for non-enrollment sends (family + their most relevant student/enrollment; portal/unsubscribe links minted per family). Marketing templates then use the whole vocabulary — names, pronouns (the PL-69 machinery), school/class facts, portal links — with save-time validation already blocking unknowns. Editor preview + test-send work unchanged (same pipeline).

**Phase C — parent+student paired sends (~½ day).** A campaign gains an audience mode: parent-only (today's default) or parent+student pair — the FO/E-series pattern: a parent template + a student template, student leg only where `student_email` exists, `campaign_recipients` grows a role column, dedupe `campaign:{id}:{email}` already keys per address. Unsubscribe/suppression stay FAMILY-level: one unsubscribe silences both legs (suppression already gates inside sendOnce).

**Phase D — the two cheapest roadmap-v2 items while the hood is open (~½ day).** (1) **Saved/named segments** (a `saved_segments` table storing the jsonb + name; live membership by construction since definitions resolve at use — the exact v2 seam v1 left); (2) **one-shot scheduled sends** (`campaigns.scheduled_for`, dispatched by the hourly sweep exactly like paused-campaign resume already is). Both were named in the batch-21 v2 roadmap; with them shipped plus one real campaign run end-to-end, **the batch-21 sunset criterion for MailerLite is met**.

**Deliberately NOT in this scope** (batch-21 v3, unchanged): multi-step drips with exit conditions (the FO sequence is now the in-house pattern to generalize from when a second sequence actually exists), A/B subjects, public capture forms, open tracking (the privacy stance decision still gates it). **No migrations needed for A/B; C and D need one small migration each.** Say go and Phase A starts the next batch.
...
**Follow-up (Aug 7, rode along in the ship commit):** the tutoring-sale sync error says "map tutoring_test_prep + tutoring_subject in the QuickBooks panel" — but the panel couldn't do it: `MAPPING_ROWS` and the `/api/qbo/mapping` allowlist only knew the three original keys, even though the DB constraint has allowed both since `20260716000001`. Both keys added to the panel (**Tutoring — test prep → QBO Item** / **Tutoring — subject → QBO Item**, hints straight from the migration's account comments: 408-1 test prep / 401 subject help) and to the API allowlist. Verified live against sandbox: both rows render their current mappings (ids 21/22), catalog loads into their dropdowns, and a save round-trips 200 through the new keys. tsc + build green.
