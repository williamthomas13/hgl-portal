...
**✅ SHIPPED (Jul 28).** New topline bar (`app/admin/topline.tsx`, rendered from the admin layout so it's on EVERY admin page): **Dashboard · Prospective Students · Tutoring · Classes · Contacts · Settings**, always clickable, each landing on its most useful default (Classes → Live class rosters, Contacts → Instructors until PL-192 hands it Students, Settings → QuickBooks). Calendar and View-as sit as utility links on the bar's right — Scarlett's six didn't file them, and they're cross-cutting views, not sections; refile on her word. The old PL-101 flat sidebar became the WITHIN-tab sub-navigation: it shows only the active tab's filed pages (Classes ← Add a new class · rosters · School contacts · Branding; Contacts ← Instructors · School contacts **second home, by design** · Communications ↗ · Agreements ↗; Settings ← QuickBooks · Google Calendar · Contact settings), and hides entirely on Dashboard. Section ids and panel-mounting are UNCHANGED (panels still hide via CSS, never unmount — the PL-99 lesson), which is what keeps every pre-restructure URL working: verified live that `/admin?class={id}` lands Classes→rosters with the class's tab selected, `?tab=classes/contacts/settings` land their defaults, `?qbo=` selects Settings→QuickBooks (signal effect), `?enrollment=` rides the same path as `?class=`, and /admin/leads · /admin/tutoring · /admin/agreements (highlights Contacts, where Scarlett filed it) each light the right topline tab. Students/Parents Contacts entries land with PL-192/193 — this is the frame.
