...
// PL-384: the ONE link model's view options — the same renderer serves both
// the internal /c/{slug} address and the evergreen /{code} address.
export type ClassPageViewOpts = {
  /** 'code' = served at the evergreen /{code} URL (canonical, indexable,
   *  register CTAs stay in the code namespace); 'slug' = the internal
   *  /c/{slug} address (ALWAYS noindex; canonical points at the class's
   *  code URL when its school/course has one). */
  mode: 'slug' | 'code'
  code?: string
}

/** The class's evergreen code (school's, else its course's) — null when none. */
async function evergreenCodeFor(cls: any): Promise<string | null> {
  const school = one<any>(cls.schools)
  if (school?.evergreen_code) return school.evergreen_code
  if (!cls.school_id && cls.course_key) {
    const { data: meta } = await supabase
      .from('course_meta')
      .select('evergreen_code')
      .eq('course_key', cls.course_key)
      .maybeSingle()
    if (meta?.evergreen_code) return meta.evergreen_code
  }
  return null
}

export async function classPageMetadata(
  slug: string,
  opts: ClassPageViewOpts = { mode: 'slug' }
): Promise<Metadata> {
  ...
  // PL-384: canonical = THE CODE URL (Scarlett's call). The /c/{slug} view is
  ...
  return {
    title,
    description,
    ...(opts.mode === 'slug' || dead ? { robots: { index: false } } : {}),
...
export async function ClassPageView({
  slug,
  opts = { mode: 'slug' },
}: {
  slug: string
  opts?: ClassPageViewOpts
}) {
  ...
  // PL-384 B: on the code-served view, registration stays at the permanent
  // address (/{code}/register) — no slug flash, links never rot.
  const registerHref = isSample
    ? '#'
    : opts.mode === 'code' && opts.code
      ? `/${opts.code}/register`
      : `/register/${cls.slug ?? cls.id}`
