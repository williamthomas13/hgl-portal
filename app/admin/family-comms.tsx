...
export type FamilyCommsItem = {
  ...
  when: string | null
  /** PL-164: lets the Communications tab's filters compose with this list. */
  templateKey?: string
}
type Item = FamilyCommsItem
...
/** PL-164: the row list + openable render, extracted so the family-record
 *  timeline and the Communications tab render the IDENTICAL implementation
 *  (one component, two surfaces — they can't drift). */
export function FamilyCommsList({ items }: { items: Item[] }) {
    ...
    <div className="text-sm">
      ...
      {open && (
        <div className="mt-2">
          ...
          {items && items.length > 0 && <FamilyCommsList items={items} />}
        </div>
      )}
    </div>
  )
