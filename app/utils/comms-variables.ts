...
export const SAMPLE_EXTRA_BY_TEMPLATE: Record<string, ExtraVars> = rehostSamples({
  ...
  AL_SWEEP_OVERDUE: {
    alertDetailsBlock:
      '<p><strong>The hourly sweep is overdue.</strong> The last completed run finished <strong>128 minutes ago</strong> (8/6/2026, 9:05:00 AM Denver).</p><p>While it\'s down, nothing sends: reminders, counselor nudges, waitlist offers, billing generation, timecard creation — the whole cadence is paused. Nothing is lost — every send is deduped and claims are retry-safe, so the next successful run delivers the backlog.</p><p><strong>To recover:</strong> re-run the sweep manually — Vercel → hgl-portal → Settings → Cron Jobs → <code>/api/cron/reminders</code> → Run, or <code>node scripts/run-sweep.mjs</code> from the repo (it sends the bearer secret from .env.local), or ask Code to hit the endpoint. The dashboard\'s health card shows live status.</p><p style="margin:20px 0"><a href="https://SAMPLE_HOST/admin" style="display:inline-block;background:#b91c1c;color:#fff;font-weight:bold;padding:12px 24px;border-radius:6px;text-decoration:none">Open the dashboard health card</a></p><p>You\'ll get one email per outage (not one per hour); the next successful sweep notes its own recovery in the dashboard activity feed.</p>',
  },
