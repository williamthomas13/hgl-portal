...
// PL-471: a quiet class never rings; the selection is its own function so
// the send projector reads the same rule the sweep does.
...
export type SynapNudgeCandidate = { id: string; label: string; createdBy: string | null; firstDay: string }

/** Classes whose Synap nudge is DUE right now (before the dedupe check). */
export async function synapNudgeCandidates(todayIso = new Date().toLocaleDateString('en-CA')): Promise<SynapNudgeCandidate[]> {
  const { data: classes } = await supabase
    .from('classes')
    .select(
      `id, class_type, delivery_mode, has_diagnostics,
...
export async function sweepSynapNudges(): Promise<number> {
  let rang = 0
  for (const c of await synapNudgeCandidates()) {
    ...
    const status = await sendAdminAlert({
      ...
      subject: `${c.label}'s Synap group still isn't set — the first diagnostic email is coming up`,
      body: `<p><strong>${c.label}</strong> was created with &ldquo;no Synap group yet&rdquo; checked, and the
        first diagnostic email is due <strong>${formatDateLong(synapAccessSendDate(c.firstDay))}</strong> (10 days before
