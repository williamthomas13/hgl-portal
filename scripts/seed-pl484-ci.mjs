#!/usr/bin/env node
// PL-484: seed CI_INTEREST_CONFIRM (approved copy) as a DRAFT for the review send → ramp from
// TEMPLATE_SEEDS as a DRAFT for the usual review send → ramp. The send engine is
// registry-only — a draft renders null and nothing sends — so flipping these
// live in the template editor IS the go-live switch (the sign-off gate).
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl484-'))
execSync(
...
const KEYS = ['CI_INTEREST_CONFIRM']
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-484: interest-list confirmation — copy approved by Scarlett Sep 21; seeded as a DRAFT for the review send → ramp; live-only render",
      created_by: 'claude',
    }])
    .select('id')
    .single()
