#!/usr/bin/env node
// PL-473/477: seed IQ_INQUIRY_ACK + PT_PARTNER_ACK (auto-reply DRAFTS for Scarlett's copy review) from
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl477-'))
execSync(
...
const KEYS = ['IQ_INQUIRY_ACK', 'PT_PARTNER_ACK']
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-473/477: auto-reply seeded as a DRAFT for copy review — registry-only, nothing sends until flipped live",
      created_by: 'claude',
    }])
    .select('id')
    .single()
