#!/usr/bin/env node
// PL-219: seed SV_CLASS_SURVEY as a DRAFT (code twin sends until Scarlett ramps it).
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl219-sv-'))
execSync(
...
for (const key of KEYS) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-219: post-class survey ask (draft — code twin sends meanwhile)',
      created_by: 'claude',
    }])
    .select('id')
    .single()
  ...
  if (after?.live !== false) {
    console.error(`FAIL  ${key} is LIVE — PL-219 requires it stay draft until approved.`)
    process.exit(1)
  }
}
