#!/usr/bin/env node
// PL-279: seed the six FO follow-on templates (3 stages × parent/student,
// Scarlett's proven copy) from TEMPLATE_SEEDS as DRAFTS. The send engine is
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl279-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-279: FO follow-on sequence — Scarlett's proven copy, seeded as a DRAFT; setting it live arms the sequence (registry-only send engine)",
      created_by: 'claude',
    }])
    .select('id')
    .single()
