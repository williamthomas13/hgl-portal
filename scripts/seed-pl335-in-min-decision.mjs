#!/usr/bin/env node
// PL-335 D: seed IN_MIN_DECISION (the instructor minimum-enrollment decision
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl335-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-335 D: instructor minimum-enrollment decision note — seeded as a DRAFT; the code twin sends the same markdown until this goes live',
      created_by: 'claude',
    }])
    .select('id')
    .single()
