...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl442-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-442: the synap-group nudge — seeded as a DRAFT; the code twin sends identical copy until this is flipped live",
      created_by: 'claude',
    }])
    .select('id')
    .single()
