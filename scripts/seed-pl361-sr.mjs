#!/usr/bin/env node
// PL-361: seed SR_PAYMENT_LINK (the staff-assisted registration payment
// link) from TEMPLATE_SEEDS as a DRAFT. The code twin in email.ts
// (staffPaymentLinkEmail) renders the same copy, so what sends before
// Scarlett flips it live is the same shape. Idempotent: byte-identical
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl361-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-361: staff-assisted registration payment link — seeded as a DRAFT; the code twin sends the same copy until this goes live',
      created_by: 'claude',
    }])
    .select('id')
    .single()
