#!/usr/bin/env node
// PL-262: seed T3_RESCHEDULE_ACK (the "got your reschedule request" ack the
// admin's Acknowledge button sends) from TEMPLATE_SEEDS as a DRAFT — the
// code twin in tutoring-emails.ts sends byte-identical copy until Scarlett
// reviews and ramps it, per the standing new-template practice.
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl262-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-262: reschedule-request acknowledgment — draft; code twin sends identical copy until ramped',
      created_by: 'claude',
    }])
    .select('id')
    .single()
