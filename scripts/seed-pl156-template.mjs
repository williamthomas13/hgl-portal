#!/usr/bin/env node
// PL-156: seed SUB_COVERAGE_NOTE as a DRAFT, and add a new DRAFT version of
// SUB_COVERAGE_RESULT carrying {coverageNoteButton}.
//
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl156-'))
execSync(
...
const KEYS = ['SUB_COVERAGE_NOTE', 'SUB_COVERAGE_RESULT']
...
for (const key of KEYS) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-156: hand-over note between the two tutors (draft — awaiting approval)',
      created_by: 'claude',
    }])
    .select('id')
    .single()
  ...
  if (after?.live !== false) {
    console.error(`FAIL  ${key} is LIVE — PL-156 requires it stay draft until approved.`)
    process.exit(1)
  }
}
