#!/usr/bin/env node
// PL-299: seed BL_BLOCK_CONFIRM (the hours-block exhaustion confirmation
// ask) from TEMPLATE_SEEDS as a DRAFT — the code twin in block-confirm.ts
// sends byte-equivalent copy until Scarlett reviews and ramps it, per the
// standing new-template practice. Idempotent.
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl299-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: 'PL-299: hours-block confirmation ask — DRAFT; the code twin sends until ramped',
      created_by: 'claude',
    }])
    .select('id')
    .single()
