#!/usr/bin/env node
// PL-461: seed T1R_UPDATED_PROPOSAL (Scarlett's approved copy) from
...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl461-'))
execSync(
...
const KEYS = ['T1R_UPDATED_PROPOSAL']
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-461: T1R updated proposal — seeded as a DRAFT (copy approved by Scarlett Sep 21); the code twin sends identical copy until this is flipped live",
      created_by: 'claude',
    }])
    .select('id')
    .single()
