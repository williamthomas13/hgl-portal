...
try {
  ...
  const pvLogo = JSON.parse(/el\.innerHTML = (".*");/.exec(await (await fetch(`${base}/embed/upcoming-classes.js?preview=upcoming2`)).text())?.[1] ?? '""')
  check('PL-508: a preview tile carries a REAL logo <img> (Nido / SAS from the schools table), not an initials box', /<img src="[^"]+" alt="Colegio Nido de Aguilas logo"/.test(pvLogo) && /<img src="[^"]+" alt="Shanghai American School logo"/.test(pvLogo))
  const testPage = await (await fetch(`${base}/sqsp-embed-test.html`)).text().catch(() => '')
  check('sqsp-embed-test.html carries the second (team) block with no heading of its own', /id="hgl-team"/.test(testPage) && /\/embed\/team\.js/.test(testPage) && !/<h2>Meet our team<\/h2>/.test(testPage))
...
if (!chromePath) {
  ...
  try {
    ...
    try {
      ...
      // PL-500 amendment: spaced like the main site — 244px circles, 182px apart at 1280 (measured there); we allow ±6px.
      const spacing = await desk.$$eval('[data-testid="team-portrait"], [data-testid="team-portrait-placeholder"]', (els) => { const rs = els.map((e) => e.getBoundingClientRect()).sort((a, b) => a.top - b.top || a.left - b.left); const row = rs.filter((r) => Math.abs(r.top - rs[0].top) < 5); const gaps = row.slice(1).map((r, i) => Math.round(r.left - row[i].right)); return { perRow: row.length, size: Math.round(rs[0].width), gaps } })
      check('/team grid at 1280: three 244px circles per row, 182px apart (the main site\'s measured spacing, PL-500)', spacing.perRow === 3 && Math.abs(spacing.size - 244) <= 6 && spacing.gaps.length === 2 && spacing.gaps.every((g) => Math.abs(g - 182) <= 6), JSON.stringify(spacing))
