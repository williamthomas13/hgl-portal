...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl429-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-429: the skipped-collateral nudge — seeded as a DRAFT; the code twin sends identical copy until this is flipped live",
      created_by: 'claude',
    }])
    .select('id')
    .single()
