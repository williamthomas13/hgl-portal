...
const out = mkdtempSync(path.join(process.cwd(), 'scripts', '.tmp-build-seed-pl424-'))
execSync(
...
if (!same) {
  ...
  const { data: inserted, error: vErr } = await db
    ...
    .insert([{
      ...
      notes: "PL-424: the availability-UPDATE alert twin — seeded as a DRAFT; the code twin sends identical copy until this is flipped live",
      created_by: 'claude',
    }])
    .select('id')
    .single()
